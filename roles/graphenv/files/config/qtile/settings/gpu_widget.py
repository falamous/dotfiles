...
class SupergfxGPU(base.InLoopPollText, base.MarginMixin, base.PaddingMixin):
    """
    ...
    def poll(self) -> str:
        gpu_type = (
            subprocess.check_output(["supergfxctl", "-g"]).decode("ascii").strip()
        )
...
class EnvyControlGPU(base.InLoopPollText, base.MarginMixin, base.PaddingMixin):
    """
    A widget to display the current gpu in use with supergxctl.
    """
    ...
    def poll(self) -> str:
        gpu_type = (
            subprocess.check_output(["envycontrol", "-q"]).decode("ascii").strip()
        )
        self.background = self.gpu_backgrounds.get(gpu_type, self._default_background)
        self.foreground = self.gpu_foregrounds.get(gpu_type, self._default_foreground)
        return self.gpu_names.get(gpu_type, gpu_type)
